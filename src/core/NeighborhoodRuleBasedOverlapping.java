 ...
 *
 */
public class NeighborhoodRuleBasedOverlapping extends Neighborhood implements INeighborhood {


	private int indexSmall, indexBig;
	private double allowedOverlap = 1;

	public NeighborhoodRuleBasedOverlapping() {
		super("Rule-based with Overlap");
	...
	public IObjectiveFunction getPreferredObjectiveFunction() {
		return new ObjectiveFunctionRBOverlap();
	}
	...
	public Neighbor getNeighbor(FeasibleSolution oldSolution) {
		...
		createOverlappingSolutionBasedOnRule(neighboredSolution);
...
//		System.out.println("[NEIGHBORHOOD] Returned solution with the following ordered set of (" + neighbor.getRectangles().size() + ") Rectangles: " + solutionListOfRectangles); // used for debugging
	 ...
	 */
	private void createOverlappingSolutionBasedOnRule(FeasibleSolution neighboredSolution) {
