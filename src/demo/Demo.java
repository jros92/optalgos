...
/**
 * A demo is a batch of instances with the same parameters, generated by the random instance generator (type 1)
 * It provides a method to sequentially solve each instance with every algorithm - neighborhood - combination available
 * in the project.
 */
public class Demo {
	...
	public Demo(int nInstances, int nRectangles, int lMin, int lMax, int lBox,
				boolean showSolutions, boolean solverLogging) {
		super();
	...
	/**
	 * Sequentially solve each instance of the demo with every algorithm - neighborhood - combination available
	 * in the project. Also stores a logfile with performance data for every run.
	 */
	public void runDemo() {
		...
		int instanceCount = 0;
		int runCount = 0;
		int totalRuns = this.nInstances * Algorithms.values().length * Neighborhoods.values().length;
		...
		for (Instance instance : instances) {
			...
			System.out.println("Demo starting to process instance "
					+ instanceCount + " of " + this.nInstances + ": " + instance.toString());
			...
			for (Algorithms algorithmChoice : Algorithms.values()) {
				for (Neighborhoods neighborhoodChoice : Neighborhoods.values()) {
					...
					System.out.println("Demo running run "
...
//					solver.setSleepDuration(0);
					...
					if (this.showSolutions) {
						FormSolutionViewer solutionViewer =
								new FormSolutionViewer(solver.getSolution(), FormMain.getDpi());
